...
先行タスクの全機能完成を待たず、公開型とFakeが渡された時点で独立検証できます。受信先の中身は固定応答で代用できますが、独自に別の共有型を作りません。本タスクのResult・Retry実装は、段階1を独立確認した[PB-TASK-0045](/tasks/prototype/pb-task-0045)の結果を受けて[PB-TASK-0046](/tasks/prototype/pb-task-0046)で実物接続を確認し、Fakeのみの確認を実接続の合格には数えません。
...
Fake Ownerの生成／解放数を検査するPlayMode試験。実Ownerへの置換後はPB-TASK-0046で再確認する。
