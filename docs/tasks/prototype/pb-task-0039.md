...
0018の[同期予約API・Fakeと引渡し状態](/tasks/prototype/pb-task-0018#handoff)を使います。判定Event／Drag Release内で一括Reservedまで確定し、受信側の判定時の音楽位置を使います。過去時刻で受付を延長せず、途中Pause／Frame切替後の元要求を新Frameへ付け替えません。同内容の処理中再入・最終結果再送・異内容矛盾は[共通境界](/spec/common-technology/feature-connections#provided-common-connections)に従います。
