...
| 担当班 | 設計書サイト（このサイト） | ページの担当班を選び、Pull Requestで直す |
| 状態・担当・期限・優先度 | Notionチケット | Notion上で動かす |

タスク名・設計書URL・明示された担当班は、マージのたびに設計書サイトの内容でNotionへ上書きされます。
...
担当班はページの作成・編集画面から選択できます。選択した班はサイドバーとNotionの両方へ反映されます。
チケットを作る前に優先度や期限を決めておきたいときは、frontmatterへ書いておくとそのまま反映されます。個人の担当割り当てはNotion上で行い、公開ページの本文・frontmatter・記入例には個人名を書きません。
...
必須の項目に加えて、次の項目を書けます。`team`は作成・編集画面で選択し、その他の項目は任意です。
...
|---|---|---|---|
| `team` | 班 | 企画／プログラム／デザイン／サウンド／全体管理 | 新規起票：プログラム／既存：現在値を保持 |
| `priority` | 優先度 | S／A／B／C | B |
| `milestone` | マイルストーン | プロトタイプ／α版／β版／完成版 | プロトタイプ |
| `assignees` | 担当 | 起票時の初期値。公開ページでは空にし、個人の割り当てはNotion上で行う | 空 |
...
::: warning 起票後の変更
`team`は設計書サイトを正とし、あとから変更した場合も次回の公開時にNotionの「班」へ反映されます。
`priority`・`milestone`・`assignees`・`due`はチケットを作るときだけ使われ、起票後はNotion上で管理します。
:::
