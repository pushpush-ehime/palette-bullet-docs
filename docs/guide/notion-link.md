...
Notion連携からDiscordへ送る通知は、**日本時間23:00以上、翌8:00未満には送信しません。** 判定にはGitHub ActionsのUTCではなく、`Asia/Tokyo`を使用します。

- 新規タスク通知：深夜にNotionチケットを起票した場合は送信せず、その通知は翌朝に再送しません。
- タスク完了通知：深夜は確認処理を抑止し、通知済みにはしません。翌8:00以降の定期実行で未通知の完了タスクを再確認します。
- 手動実行：GitHub Actionsを深夜に手動実行しても、スクリプト側の判定でDiscord Webhookを呼びません。

完了通知の定期実行は、毎時00分付近の混雑を避け、日本時間08:30〜22:30の毎時30分に行います。
...
| 実装・成果記録 | GitHubのタスクMarkdown | 途中経過・成果を追記してPull Requestで確認する |

タスク名・設計書URL・明示された担当班は、マージのたびに設計書サイトの内容でNotionへ上書きされます。
状態・担当・期限・優先度は起票時に一度書き込むだけで、そのあとは自動で変わりません。

Webのタスク上部・一覧には、Notionから取得した進捗とJST取得日時を表示します。
通常公開と毎日12:00 JSTの日次更新で反映し、リアルタイム表示ではありません。
日次処理は読み取り専用で、起票やDiscord通知は行いません。
取得失敗時・未連携時の意味と実装記録の書式は[タスクの進捗と実装・成果記録](/guide/task-records)を参照してください。
...
- **Notion側が不調のとき**：連携だけ飛ばして公開は続きます。進捗とリンクは利用できる前回取得情報を使い、元の日時と前回情報である旨を表示します。前回情報がなければ状態不明です。
