...
[拠点の町](/spec/hub/)と[演奏広場](/spec/hub/performance-plaza)は別の制作範囲です。ラジクジラの[初期追従パターン](/spec/radiowhale/behavior-patterns)は初期の接続で使用できますが、ストーリークリア後の行動設定画面、追加パターン、広場演奏を本ページの完成条件へ追加しません。
...
段階1の実装範囲・Sceneの作成単位・機能ごとの受け渡しは[プロトタイプタスク](/tasks/prototype/)へ具体化しています。検証用素材・仮パラメータの採用値、未割当の個人と日付は別途確認し、この段階表から推定して埋めません。
...
公開Web仕様書には役割・担当範囲を記載し、個人名は掲載しません。具体的な担当者の割り当ては、チーム内のNotionで管理します。

| 役割 | 公開する窓口 | 担当範囲 |
|---|---|---|
| 接続実装の取りまとめ | プログラマー班の接続実装担当 | 仕様書と割り当てられた実装タスクに基づき、通常のCharge→発射→浄化のコード・グラフ接続と動作検証を進める。6枠の機能別分担、枠1の共有Scene、枠4のPlayer共有アセットの窓口は[プロトタイプタスク](/tasks/prototype/#owners)を正とする |
| 操作結果の確認 | 完成方針・仕様の判断窓口 | 実際の操作結果が意図した体験と合格条件に合うか確認する |
| コードのPR確認 | 未定 | コード・グラフ変更のレビュー担当は、操作結果の確認担当と区別して後続で決める |

具体的な作業時間と期限、コードレビュー・マージ担当は未確定です。後続で承認した6枠の実装タスク、枠1の共有Scene、枠4のPlayer Prefab／Input／共通グラフの取りまとめは、[プロトタイプタスクの窓口](/tasks/prototype/#owners)を参照します。個人への割当をこのページへ重複記載しません。
...
必須機能の準備・Gameplay cleanupの失敗、および追加採用した実行中の内部異常は、[共通接続仕様D03](/spec/common-technology/feature-connections#failure-policy)に従い、失敗した機能・理由と再起動案内を表示して受入確認を中断します。通常miss・通常の対象消失・任意読取失敗との境界もD03を参照します。再試行はアプリを終了して起動し直す方式とし、エラー画面からの復旧は今回の必須範囲に含めません。異常時の表示・中断を確認できても、失敗した通常プレイのケースを合格にはしません。正常なGame OverからのResult Retryは従来どおり今回の必須機能です。
...
今回合意した2点は[完成方針](#completion-policy)を参照します。Sceneの作成単位、6枠の役割分担、共有Scene／Player共有アセットの取りまとめ窓口、段階1と終了・Result・Retryの統合区分は[プロトタイプタスク](/tasks/prototype/)で決定済みです。次の具体化だけを未決として扱います。

- PB-TASK-0018で確定する共通APIの公開名・型・単位・有効期間・Owner・呼出し順、およびassembly／テストの参照構成。0018のレビュー済み引渡し前に他の実装タスクを正式着手しない
- 段階1に使用する検証用BGM／MusicChart、Enemy、Stage配置と仮パラメータの採用値
- Mode 2～4の固定プリセットの効果・数値・音の違い、および確認ケース
- 枠2～6の個人担当、各タスクの確認担当と期限。公開仕様から個人名や稼働時間を推測しない
- コード／グラフのPR確認・マージを行う個人。仕様判断と操作結果の確認窓口は決定済みの役割を使う
