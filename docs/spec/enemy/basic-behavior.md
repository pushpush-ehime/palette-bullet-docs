...
- **パリィ**：Parry成功した各邪音玉はPlayerへのDamageを発生させず、邪音玉としての攻撃projectile機能を終了し、Parry成立時点のworld位置で同じ`battleId`のWildcard 1個へ即時変換される。変換後のWildcardは変換commitと同時に選択可能になり、その後、Parryで対象ごとに確定した弾き方向の力を一度だけ受け、選択可能なWildcardとして少し移動する。移動するのは反射された邪音玉ではなく、変換済みのWildcardである。同一Physics Stepに接触した複数弾の成功batch判定と対象ごとの弾き方向の確定は[Playerアクション｜パリィ](/spec/player/player-action-parry)、各邪音玉のDamage無効化・projectile終了・弾ごとの変換要求は[Jaon Bullet](/spec/enemy/jaon-bullet)、即時変換と選択可能化は[万能シャオンダマ](/spec/shaondama-music/wildcard-orb)、力を受けた後の移動・浮遊・共通lifecycleは[浮遊挙動](/spec/shaondama-music/floating-behavior)を正本とする
