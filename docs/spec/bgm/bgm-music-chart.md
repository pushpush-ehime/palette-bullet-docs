...
音楽制作側が曲ごとの順序を提示し、Gameplay上の採用はプランナーが決めます。2-Aの固定fixtureの順序指定は、その曲別判断を完了した証拠ではありません。0031がこの読取を使う本番検索／配送を実装します。BindingはChart参照とNote fingerprintを記録し、再ImportなどによってChart参照またはfingerprint対象データが変わった場合、順序読取時に不一致を検出します。対象データはPPQN、順序付きTrack ID・Track／Note数、各Noteの番号・velocity・startTick・lengthTicksです。同一Assetへの同一対象データの再Importは不一致にならず、Tempoイベント・Audio・Attack・Gameplay設定だけの変更もこのfingerprint比較の対象外です。再Import操作そのものを監視する仕組みではありません。これとは別に、作者が明示する再確認／reset操作（`ResetOrdersForSource`）は旧順序をすべて破棄して元データを記録し直します。旧順序を自動再承認しません。
