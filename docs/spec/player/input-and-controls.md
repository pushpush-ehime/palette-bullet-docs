...
| コンダクト選択 | 未選択を含むコンダクトを循環選択する | マウスホイール回転 | 入力割当は確定 |
...
モード1～4は、`1`～`4`で目的のモードを直接選択します。順送り入力ではありません。入力時に即座に切り替えず、次の対象小節頭へのMode変更を要求します。適用前の有効入力は最後に受理したModeをpendingとして保持します。
