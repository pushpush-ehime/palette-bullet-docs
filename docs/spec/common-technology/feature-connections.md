...
| C02 | 全必須Ready成立 → C03へ。開始前の解除は待機へ戻す | 旧ID・重複Readyで条件を水増ししない。準備失敗の表示・再試行方針はQ06 | 最後のReady直前にSupply Ready解除 → 開始しない。復帰後に一度だけ開始 |
...
| C17 | 結果を一度確定 → gate閉鎖 → 各Owner停止・終了集約 | 必須cleanup失敗なら解禁しない。再通知で二重解放しない。表示用残像はGameplay無効 | 1 Ownerのcleanup失敗 → Resultを表示できても操作はlock、失敗理由が残る |
...
ここからは、既存契約をつなぐための**技術提案**です。今回採用済みのD01・D02や既存仕様の事実と区別します。通常のC#メソッド・イベント・既存Playerの入口で実現できる構成を優先します。
...
- 必須cleanupに失敗した場合は失敗Ownerと原因を保持して解禁しない。単なるタイムアウトで成功扱いにしない。ユーザーへ提示する中断・再試行操作はQ06。
...
以下は実装差分の一覧です。今回、コード変更・タスク発行・担当割り当ては行いません。
...
- 必須cleanup集約、Result操作解禁、旧Battle通知拒否、失敗時の表示。
...
| ページ間の矛盾が残るか | Q07～Q09などを明示。D01・D02は今回Playerと関連する正本へ反映。他の未合意案で既存仕様を上書きしない |
