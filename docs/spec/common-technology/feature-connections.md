...
既存の[Battle／Result／Retryタスク](/tasks/game/pb-task-0018)、[Windows配布タスク](/tasks/game/pb-task-0017)、[受入計画タスク](/tasks/game/pb-task-0024)は更新・分割の照合先です。基盤の実装済み部分を再発行せず、担当間の接続、全体動作、Windows配布受入も候補へ含めます。この節自体ではタスク発行・担当割り当てを行いません。
