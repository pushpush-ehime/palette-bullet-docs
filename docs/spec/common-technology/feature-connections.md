...
Q01～Q05の共通境界は、それを使う実装タスクの分配前に具体化する優先事項です。この具体化を行う最初の実装タスクとして、PB-TASK-0018だけを先に正式配分します。0018が正確な公開名・型・単位・有効期間・Owner・呼出し順、コンパイル可能なFakeと呼出し例、assemblyとテストの参照構成をPRで提示し、レビュー済みの引渡しCommitを記録するまで、他のプロトタイプ実装タスクは正式着手しません。待機中の担当候補は仕様確認・既存コード調査・機能内部の設計まで進められますが、独自の共有型や接続口を確定しません。

通常の技術判断は仕様作成側が既存実装に沿った案をまとめ、ゲーム上の意味が変わる点だけを協議します。0018担当は決められた意味を型付きコードへ落とし込み、レビューで接続契約を確定します。プログラマーの成果物はコード・グラフ・アセットと動作確認であり、仕様の表作成を実装前提の担当作業にはしません。
